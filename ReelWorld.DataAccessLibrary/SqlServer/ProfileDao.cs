        ...
        public async Task<Profile?> GetOneAsync(int profileId)
        {
            ...
            try
            {
                ...
                SELECT 
                    ...
                    COALESCE(STRING_AGG(i.InterestName, ', '), '') AS Interests
                FROM Profile p
                ...
                if (result != null)
                {
                    result.AccessLevel = new AccessLevel
                    {
                        AccessLevelId = result.FK_AccessLevel_Id ?? 0,
                        Name = result.AccessLevelName,
                        Description = result.AccessLevelDescription
                    };
                }
